...
def save_trade(symbol, action, price, rsi, ema20, ema50, volume, result):
...
def get_signal_data(ticker):
    ...
    return price, rsi_value, ema20, ema50


def run_coin(coin):
    symbol = coin["symbol"]

    price, rsi_value, ema20, ema50 = get_signal_data(coin["ticker"])
    balance = get_balance(coin["balance_key"])

    print(f"{symbol} Price:", price)
    print(f"{symbol} RSI:", rsi_value)
    print(f"{symbol} EMA20:", ema20)
    print(f"{symbol} EMA50:", ema50)
